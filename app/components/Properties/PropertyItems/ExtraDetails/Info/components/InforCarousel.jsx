...
const InfoCarousel = ({ details }) => {
  ...
  const getTagWithIcon = (tag) => {
    const matchedTag = allTags.find((item) => item.title === tag);
          ...
          {chunkedDetails.map((chunk, chunkIndex) => (
            ...
            >
              {chunk.map((detail, index) => (
                ...
                >
                  <span className="flex w-min items-center gap-1 whitespace-nowrap rounded bg-[#f5fbff] px-2 py-[2px] text-[14px]">
                    <span className="text-[12px]">
                      {getTagWithIcon(detail.tag)}
                    </span>
                    {detail.tag}
                  </span>
                  <div>
