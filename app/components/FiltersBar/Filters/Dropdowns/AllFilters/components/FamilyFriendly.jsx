...
export default function FamilyFriendly({ filter, selectedFilter }) {
  const {
          ...
          <ul className="grid grid-cols-2 gap-2">
            {familyFriendlyOptions.map((option) => (
              <li key={uuidv4()} className="w-full">
                <CheckboxItem
                  checked={localSelectedFamilyFriendlyOptions.includes(option)}
                  label={option}
                  onChange={() => toggleFamilyFriendlySelection(option)}
                />
              </li>
            ))}
          </ul>
